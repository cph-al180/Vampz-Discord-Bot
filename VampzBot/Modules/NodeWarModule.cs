    ...
    public class NodeWarModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task JoinNodeWar(string gearScore, string playerClass, string level)
        {
            ...
            catch (Exception e)
            {
                ...
                await ReplyAsync("Error. Please contact: " + Context.Guild.GetUser((ulong)SecretManager._secrets.Taki).Mention);
            }
        }
        ...
        public async Task ViewSignUps()
        {
            ...
            else if (signedMembers.Count == 0)
            {
                await ReplyAsync(Context.Message.Author.Mention + " No sign ups so far");
            }
            ...
            else
            {
                ...
                builder.AddField("Signups " + signedMembers.Count, participants);
                builder.WithColor(Color.Red);
